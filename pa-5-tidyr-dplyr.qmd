...
military <- read_xlsx("data/gov_spending_per_capita.xlsx",  #selects data file
                      sheet = "Share of Govt. spending",    #selects specific sheet
                      skip  = 7,                            #skips first 7 rows
                      n_max = 191,                          #sets max number of observations
                      na = c("xxx", ". ."))                  #sets NA values
```
...
>This code filters the data down to just countries, and removes regions from the dataset.
...
military_clean |> 
  filter(if_all(.cols = 2:35,   #hint: what is the easiest way to include every column except `Country`
